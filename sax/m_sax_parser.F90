...
contains
  ...
  recursive subroutine sax_parse(fx, fb, &
    ...
    do
      ...
      case (ST_MISC)
        !write(*,*) 'ST_MISC', as_chars(fx%token)
      ...
      case (ST_START_PI)
        ...
        case (TOK_NAME)
          if (namespaces_) then
            nameOk = checkNCName(as_chars(fx%token), fx%xds%xml_version)
          else
            nameOk = checkName(as_chars(fx%token), fx%xds%xml_version)
          endif
          if (nameOk) then
            if (fx%token=='xml') then
              call add_error(fx%error_stack, "XML declaration must be at start of document")
              goto 100
            elseif (checkPITarget(as_chars(fx%token), fx%xds%xml_version)) then
      ...
      case (ST_START_TAG)
        ...
        case (TOK_NAME)
          if (fx%context==CTXT_BEFORE_DTD &
            ...
            if (namespaces_) then
              nameOk = checkQName(as_chars(fx%token), fx%xds%xml_version)
            else
              nameOk = checkName(as_chars(fx%token), fx%xds%xml_version)
      ...
      case (ST_IN_TAG)
        ...
        case (TOK_NAME)
          ...
          if (has_key(fx%attributes, as_chars(fx%token))) then
          ...
          if (associated(elem)) then
            attDecl => get_attribute_declaration(elem, as_chars(fx%attname))
          else
            attDecl => null()
          endif
      ...
      case (ST_ATT_EQUALS)
        ...
        case (TOK_CHAR)
          ...
          if (temp_i==ATT_CDATA) then
            call add_item_to_dict(fx%attributes, as_chars(fx%attname), &
              as_chars(fx%token), itype=ATT_CDATA, declared=associated(attDecl))
          else
            if (validCheck) then
              if (fx%xds%standalone.and..not.attDecl%internal &
                .and.(as_chars(fx%token)//"x"/=att_value_normalize(as_chars(fx%token))//"x")) then
                call add_error(fx%error_stack,  &
              ...
              if (elementContent(fx%elstack)) then
                if (verify(as_chars(fx%token), XML_WHITESPACE)==0) then
                  ...
                  if (present(ignorableWhitespace_handler)) then
                    call ignorableWhitespace_handler(as_chars(fx%token))
                    if (fx%state==ST_STOP) goto 100
                  endif
                else
      ...
      case (ST_START_ENTITY)
        ...
        case (TOK_NAME)
          ...
          ent => getEntityByName(fx%forbidden_ge_list, as_chars(fx%token))
          if (associated(ent)) then
            call add_error(fx%error_stack, 'Recursive entity reference')
            goto 100
          endif
          ent => getEntityByName(fx%predefined_e_list, as_chars(fx%token))
          if (associated(ent)) then
            if (present(startEntity_handler)) then
              call startEntity_handler(as_chars(fx%token))
              if (fx%state==ST_STOP) goto 100
            endif
            if (validCheck) then
              if (associated(elem)) then
            ...
            if (present(characters_handler)) then
              call characters_handler(expand_entity(fx%predefined_e_list, as_chars(fx%token)))
              if (fx%state==ST_STOP) goto 100
            endif
            if (present(endEntity_handler)) then
              call endEntity_handler(as_chars(fx%token))
              if (fx%state==ST_STOP) goto 100
            endif
          elseif (likeCharacterEntityReference(as_chars(fx%token))) then
            if (checkRepCharEntityReference(as_chars(fx%token), fx%xds%xml_version)) then
              if (validCheck) then
                if (associated(elem)) then
                  if (.not.elem%mixed.and..not.elem%any) then
                    call add_error(fx%error_stack, &
              ...
              if (present(characters_handler)) then
                call characters_handler(expand_char_entity(as_chars(fx%token)))
                if (fx%state==ST_STOP) goto 100
              endif
            elseif (checkCharacterEntityReference(as_chars(fx%token), fx%xds%xml_version)) then
          ...
          elseif (existing_entity(fx%xds%entityList, as_chars(fx%token))) then
            ent => getEntityByName(fx%xds%entityList, as_chars(fx%token))
            ...
            elseif (ent%external) then
              ...
              if (iostat/=0) then
                if (validCheck) then
                  call add_error(fx%error_stack, &
                    "Unable to retrieve external entity "//as_chars(fx%token))
                  goto 100
                endif
                if (present(skippedEntity_handler)) then
                  call skippedEntity_handler(as_chars(fx%token))
                  if (fx%state==ST_STOP) goto 100
                endif
              else
                if (present(startEntity_handler)) then
...
#ifdef PGF90
                call add_internal_entity(fx%forbidden_ge_list, as_chars(fx%token), "", nullURI, .false.)
#else
                call add_internal_entity(fx%forbidden_ge_list, as_chars(fx%token), "", null(), .false.)
#endif
                temp_wf_stack => wf_stack
                allocate(wf_stack(size(temp_wf_stack)+1))
              ...
              call add_internal_entity(fx%forbidden_ge_list, as_chars(fx%token), "", null(), .false.)
#endif
              call open_new_string(fb, expand_entity(fx%xds%entityList, as_chars(fx%token)), &
                                  &  as_chars(fx%token), baseURI=ent%baseURI)
              temp_wf_stack => wf_stack
              allocate(wf_stack(size(temp_wf_stack)+1))
              wf_stack = (/0, temp_wf_stack/)
              deallocate(temp_wf_stack)
      ...
      case (ST_CLOSING_TAG)
        ...
        select case (fx%tokenType)
        case (TOK_NAME)
          if (checkName(as_chars(fx%token), fx%xds%xml_version)) then
            fx%name => fx%token
            nullify(fx%token)
      ...
      case (ST_DOC_NAME)
        !write(*,*) 'ST_DOC_NAME ', as_chars(fx%token)
        select case (fx%tokenType)
        case (TOK_NAME)
          if (fx%token=='SYSTEM') then
            nextState = ST_DOC_SYSTEM
          elseif (fx%token=='PUBLIC') then
        ...
        case (TOK_OPEN_SB)
          if (present(startDTD_handler)) then
            if (associated(fx%publicId)) then
              call startDTD_handler(as_chars(fx%root_element), &
                publicId=as_chars(fx%publicId), systemId=as_chars(fx%systemId))
            elseif (associated(fx%systemId)) then
              call startDTD_handler(as_chars(fx%root_element), &
                publicId="", systemId=as_chars(fx%systemId))
            else
              call startDTD_handler(as_chars(fx%root_element), "", "")
            endif
            if (fx%state==ST_STOP) goto 100
          ...
          if (associated(fx%systemId)) then
            extSubsetURI => parseURI(as_chars(fx%systemId))
            call destroy_vs(fx%systemId)
          endif
          if (associated(fx%publicId)) call destroy_vs(fx%publicId)
          fx%inIntSubset = .true.
          wf_stack(1) = wf_stack(1) + 1
        ...
        case (TOK_END_TAG)
          ...
          if (associated(fx%systemId)) then
            ...
            if (iostat==0) then
              ...
              if (validCheck) then
                call add_error(fx%error_stack, &
                  "Unable to retrieve external subset "//as_chars(fx%systemId))
                goto 100
              endif
              call endDTDchecks
              if (in_error(fx%error_stack)) goto 100
      ...
      case (ST_START_PE)
        ...
        select case (fx%tokenType)
        case (TOK_NAME)
          if (existing_entity(fx%forbidden_pe_list, as_chars(fx%token))) then
            call add_error(fx%error_stack, &
              'Recursive entity reference')
            goto 100
          endif
          ent => getEntityByName(fx%xds%PEList, as_chars(fx%token))
          if (associated(ent)) then
            if (ent%wfc.and.fx%xds%standalone) then
              call add_error(fx%error_stack, &
            ...
            elseif (ent%external) then
              ...
              call add_internal_entity(fx%forbidden_pe_list, &
                as_chars(fx%token), "", nullURI, .false.)
#else
              call add_internal_entity(fx%forbidden_pe_list, &
                as_chars(fx%token), "", null(), .false.)
#endif
              call open_new_file(fb, ent%baseURI, iostat, pe=.true.)
              if (iostat/=0) then
                if (validCheck) then
                  call add_error(fx%error_stack, &
                    "Unable to retrieve external parameter entity "//as_chars(fx%token))
                  goto 100
                endif
                if (present(skippedEntity_handler)) then
                  call skippedEntity_handler('%'//as_chars(fx%token))
              ...
              call open_new_string(fb, &
                expand_entity(fx%xds%PEList, as_chars(fx%token)), as_chars(fx%token), baseURI=nullURI, pe=.true.)
#else
              call add_internal_entity(fx%forbidden_pe_list, &
                as_chars(fx%token), "", null(), .false.)
              call open_new_string(fb, &
                expand_entity(fx%xds%PEList, as_chars(fx%token)), as_chars(fx%token), baseURI=null(), pe=.true.)
#endif
  ...
  contains
    ...
    subroutine parseDTD
      ...
      case (ST_DTD_BANG_TAG)
        ...
        case (TOK_NAME)
          if (fx%token=='ATTLIST') then
            nextDTDState = ST_DTD_ATTLIST
          elseif (fx%token=='ELEMENT') then
            nextDTDState = ST_DTD_ELEMENT
          elseif (fx%token=='ENTITY') then
            nextDTDState = ST_DTD_ENTITY
          elseif (fx%token=='NOTATION') then
      ...
      case (ST_DTD_ATTLIST)
        ...
        case (TOK_NAME)
          ...
          if (existing_element(fx%xds%element_list, as_chars(fx%token))) then
            elem => get_element(fx%xds%element_list, as_chars(fx%token))
          else
            elem => add_element(fx%xds%element_list, as_chars(fx%token))
          endif
      ...
      case (ST_DTD_ATTLIST_CONTENTS)
        ...
        case (TOK_DTD_CONTENTS)
          if (processDTD) then
            call parse_dtd_attlist(as_chars(fx%token), fx%xds%xml_version, &
      ...
      case (ST_DTD_ELEMENT_CONTENTS)
        ...
        case (TOK_ENTITY)
          ...
          nextState = ST_START_PE
        case (TOK_DTD_CONTENTS)
          if (declared_element(fx%xds%element_list, as_chars(fx%name))) then
            if (validCheck) then
          ...
          elseif (processDTD) then
            if (existing_element(fx%xds%element_list, as_chars(fx%name))) then
              elem => get_element(fx%xds%element_list, as_chars(fx%name))
            else
              elem => add_element(fx%xds%element_list, as_chars(fx%name))
            endif
          else
            elem => null()
          endif
          call parse_dtd_element(as_chars(fx%token), fx%xds%xml_version, fx%error_stack, &
      ...
      case (ST_DTD_ENTITY_ID)
        ...
        select case (fx%tokenType)
        case (TOK_NAME)
          if (fx%token == "PUBLIC") then
            nextDTDState = ST_DTD_ENTITY_PUBLIC
          elseif (fx%token == "SYSTEM") then
      ...
      case (ST_DTD_NOTATION_PUBLIC_2)
        ...
        case (TOK_END_TAG)
          if (validCheck) then
            ...
            if (notation_exists(fx%nlist, as_chars(fx%name))) then
      ...
      case (ST_DTD_NOTATION_END)
        ...
        case (TOK_END_TAG)
          ...
          wf_stack(1) = wf_stack(1) - 1
          if (processDTD) then
            URIref => parseURI(as_chars(fx%systemId))
    ...
    subroutine open_tag
      ...
      if (associated(elem)) then
        if (validCheck) then
          ...
          if (.not.checkContentModel(fx%elstack, as_chars(fx%name))) then
            call add_error(fx%error_stack, &
              "Element "//as_chars(fx%name)//" not permitted in this context")
            return
          endif
        else
      ...
      if (namespaces_.and.getURIofQName(fx,as_chars(fx%name))==invalidNS) then
        ...
        elseif (present(startElement_handler)) then
          ...
          call startElement_handler("", &
            getlocalNameofQName(as_chars(fx%name)), &
            as_chars(fx%name), fx%attributes)
          if (fx%state==ST_STOP) return
        endif
      elseif (namespaces_) then
        ! Normal state of affairs
        if (present(startElement_handler)) then
          call startElement_handler(getURIofQName(fx, as_chars(fx%name)), &
          getlocalNameofQName(as_chars(fx%name)), &
          as_chars(fx%name), fx%attributes)
          if (fx%state==ST_STOP) return
        endif
      ...
      if (validCheck) then
        call push_elstack(fx%elstack, as_chars(fx%name), elem%cp)
      else
        call push_elstack(fx%elstack, as_chars(fx%name))
      endif
    ...
    subroutine close_tag
      ...
      if (validCheck) then
        if (.not.checkContentModelToEnd(fx%elstack)) then
          call add_error(fx%error_stack, &
            "Failed to fulfil content model for "//as_chars(fx%name))
          return
        endif
      endif
      ...
      if (present(endElement_handler)) then
        ...
        elseif (namespaces_) then
          ! Normal state of affairs
          call endElement_handler(getURIofQName(fx, as_chars(fx%name)), &
                & getlocalnameofQName(as_chars(fx%name)), &
                & as_chars(fx%name))
        else
          ! Non-namespace-aware processing:
    ...
    subroutine add_entity
      ...
      if (pe) then
        ...
        if (.not.existing_entity(fx%xds%PEList, as_chars(fx%name))) then
          ! Internal or external?
          if (associated(fx%attname)) then ! it's internal
            call register_internal_PE(fx%xds, &
              name=as_chars(fx%name), text=as_chars(fx%attname), &
            ...
            if (present(internalEntityDecl_handler)) then
              call internalEntityDecl_handler('%'//as_chars(fx%name), as_chars(fx%attname))
              if (fx%state==ST_STOP) return
            endif
            ...
            elseif (hasFragment(URIref)) then
              ...
              if (associated(fx%publicId)) then
                ...
                if (present(externalEntityDecl_handler)) &
                  call externalEntityDecl_handler('%'//as_chars(fx%name), &
                  systemId=as_chars(fx%systemId), publicId="")
              endif
            endif
          endif
          ! else we ignore it
        ...
        if (.not.existing_entity(fx%xds%entityList, as_chars(fx%name))) then
          ! Internal or external?
          if (associated(fx%attname)) then ! it's internal
            call register_internal_GE(fx%xds, name=as_chars(fx%name), &
              text=as_chars(fx%attname), &
              wfc=wfc, baseURI=copyURI(fb%f(1)%baseURI))
            if (present(internalEntityDecl_handler)) then
              call internalEntityDecl_handler(as_chars(fx%name),&
              as_chars(fx%attname))
              if (fx%state==ST_STOP) return
            endif
          else
            ...
            elseif (hasFragment(URIref)) then
              ...
              elseif (associated(fx%publicId)) then
                ...
                call register_external_GE(fx%xds, name=as_chars(fx%name), &
                  systemId=as_chars(fx%systemId), wfc=wfc, baseURI=newURI)
                if (present(externalEntityDecl_handler)) &
                  call externalEntityDecl_handler(as_chars(fx%name), &
                  systemId=as_chars(fx%systemId), publicId="")
              endif
