...
define(`TOHWM4_format_real', `dnl

  pure function real_$1_str(x, sig) result(s)
    real($1), intent(in) :: x
  ...
  end function real_$1_str


  pure function str_real_$1_fmt_len(x, fmt) result(n)
    real($1), intent(in) :: x
    ...
    elseif (fmt(1:1) == "r") then
      ...
      dec = min(dec, digits(1.0_$1)-e)
      dec = max(dec, 0)

      if (dec > 0) n = n + 1
      if (abs(x) >= 1.0_$1) n = n + 1

      ! Need to know if there"s an overflow ....
      if (e+dec+1 > 0) then
        if (index(real_$1_str(abs(x), e+dec+1), "!") == 1) &
             e = e + 1
      endif
  ...
  end function str_real_$1_fmt_len


  function str_real_$1_fmt(x, fmt) result(s)
    real($1), intent(in) :: x
    character(len=*), intent(in) :: fmt
    character(len=str_real_$1_fmt_len(x, fmt)) :: s

    integer :: sig, dec
    integer :: e, n
    character(len=str_real_$1_fmt_len(x, fmt)) :: num !this wll always be enough memory.
    ...
    elseif (fmt(1:1) == "r") then
      ...
      dec = min(dec, digits(1.0_$1)-e-1)
      dec = max(dec, 0)

      if (e+dec+1 > 0) then
        num = real_$1_str(abs(x), e+dec+1)
      else
        num = ""
      endif
      if (num(1:1) == "!") then
  ...
  end function str_real_$1_fmt


  pure function str_real_$1_len(x) result(n)
    real($1), intent(in) :: x
    integer :: n

    n = str_real_$1_fmt_len(x, "")

  end function str_real_$1_len


  function str_real_$1(x) result(s)
    real($1), intent(in) :: x
    character(len=str_real_$1_len(x)) :: s

    s = str_real_$1_fmt(x, "")

  end function str_real_$1

     
  pure function str_real_$1_array_len(xa) result(n)
    real($1), dimension(:), intent(in) :: xa
    integer :: n
    ...
    n = size(xa) - 1
    do k = 1, size(xa)
      n = n + str_real_$1_fmt_len(xa(k), "")
    enddo
    
  end function str_real_$1_array_len


  function str_real_$1_array(xa) result(s)
    real($1), dimension(:), intent(in) :: xa
    character(len=str_real_$1_array_len(xa)) :: s
    
    integer :: j, k, n

    n = 1
    do k = 1, size(xa) - 1
      j = str_real_$1_fmt_len(xa(k), "")
      s(n:n+j) = str(xa(k), "")//" "
      n = n + j + 1
    enddo
    s(n:) = str(xa(k))

  end function str_real_$1_array

 
  pure function str_real_$1_array_fmt_len(xa, fmt) result(n)
    real($1), dimension(:), intent(in) :: xa
    character(len=*), intent(in) :: fmt
    integer :: n
    ...
    n = size(xa) - 1
    do k = 1, size(xa)
      n = n + str_real_$1_fmt_len(xa(k), fmt)
    enddo
    
  end function str_real_$1_array_fmt_len
     

  function str_real_$1_array_fmt(xa, fmt, delimiter) result(s)
    real($1), dimension(:), intent(in) :: xa
    character(len=*), intent(in) :: fmt
    character(len=1), intent(in), optional :: delimiter
    character(len=str_real_$1_array_fmt_len(xa, fmt)) :: s
    ...
    n = 1
    do k = 1, size(xa) - 1
      j = str_real_$1_fmt_len(xa(k), fmt)
      s(n:n+j) = str(xa(k), fmt)//d
      n = n + j + 1
    enddo
    s(n:) = str(xa(k), fmt)

  end function str_real_$1_array_fmt


  function str_real_$1_matrix(xa) result(s)
    real($1), dimension(:,:), intent(in) :: xa
    character(len=str_real_$1_array_len(reshape(xa,(/size(xa)/)))) :: s

    s = str_real_$1_array(reshape(xa,(/size(xa)/)))
  end function str_real_$1_matrix
    

  function str_real_$1_matrix_fmt(xa, fmt) result(s)
    real($1), dimension(:,:), intent(in) :: xa
    character(len=*), intent(in) :: fmt
    character(len=str_real_$1_array_fmt_len(reshape(xa,(/size(xa)/)),fmt)) :: s

    s = str_real_$1_array(reshape(xa,(/size(xa)/)))
  end function str_real_$1_matrix_fmt
')dnl
dnl
!
! This file is AUTOGENERATED
! To update, edit m_common_format.m4 and regenerate
...
contains
  ...
  TOHWM4_format_real(`sp')

  TOHWM4_format_real(`dp')
