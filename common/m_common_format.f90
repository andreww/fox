  ...
  ! to discover for an arbitrary . Therefore, 
  ...
  !  includes whitespace, this will be lost.)
  !
  ...
  ! The standard Fortran  functions do not give us
  ...
  ! for that real type.
  ! The absence of a  implies scientific notation, with
  ...
  ! Ideally they'd be rewritten to convert from teh native  by
  ! bit-twidding. Not sure how to do that portably though.

  ! The  specification could be done more nicely - but unfortunately
  ! not in F95 due to *stupid* restrictions on specification expressions.

  ! And I wouldn't have to invent my own  specification if Fortran
...
contains
  ...
  function str_real_dp_fmt(x, fmt) result(s)
    ...
    integer :: sig, dec
    integer :: e, n
    character(len=str_real_dp_fmt_len(x, fmt)) :: num !this wll always be enough memory.
